...
const COMPLETED_STATUSES: LoadStatus[] = [
  "delivered",
  "pod_received",
  "invoiced",
  "paid",
];
const CORE_CHECKIN_ORDER: CheckinEvent[] = [
  "arrived_pickup",
  "loaded_departing",
  "arrived_delivery",
  "delivered",
...
> = {
  ...
  detention_start: {
    label: "Start Detention",
    subLabel: "Waiting at facility",
    Icon: Timer,
    color: "text-black",
  ...
  tonu: {
    label: "TONU",
    subLabel: "Truck Ordered Not Used",
    Icon: XCircle,
    color: "text-white",
        ...
        <div className="p-5 border-b border-white/5">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              {cfg.subLabel && (
                <div className="text-slate-400 text-xs">{cfg.subLabel}</div>
              )}
            </div>
          </div>
        </div>
    ...
    <div className="mt-2">
      ...
      <div className="flex flex-wrap gap-1.5">
        {photos.map((p) => (
          <button
            key={p.id}
            onClick={() => openDocument(p.url)}
            className="w-14 h-14 rounded-xl overflow-hidden relative group text-left"
...
function UploadSlotCard({
  ...
  const slotPhotos = photos.filter((p) =>
    p.caption?.startsWith(`[${slot.key}]`),
  );

  // For docField slots, check if the doc URL exists on the load
  const docUrl = slot.docField
    ? ((load as any)[slot.docField] as string | undefined)
    : undefined;
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      if (slot.docField) {
        ...
        const updateData: Partial<SonexLoad> = {
          [slot.docField]: result.url,
        } as any;
  ...
  return (
    <div
      className={`rounded-xl p-3 transition-all ${!unlocked ? "opacity-40" : ""}`}
      style={{
        background: hasContent ? slot.bg : "rgba(255,255,255,0.025)",
        border: `1px solid ${hasContent ? slot.border : "rgba(255,255,255,0.06)"}`,
      }}
    >
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-1 flex-shrink-0">
              ...
              <button
                ...
                title={
                  unlocked
                    ? `Upload ${slot.label}`
                    : `Complete: ${slot.unlock.map((e) => CHECKIN_EVENT_LABELS[e]).join(", ")} first`
                }
              >
                {uploading ? (
          ...
          {hasContent && !slot.docField && (
            <div className="text-[10px] text-emerald-400 mt-1 flex items-center gap-1">
              <CheckCircle size={9} /> {slotPhotos.length} photo
              {slotPhotos.length !== 1 ? "s" : ""} uploaded
            </div>
          )}
          {docUrl && (
            <div className="text-[10px] text-emerald-400 mt-1 flex items-center gap-1">
          ...
          {!unlocked && (
            <div className="text-[10px] text-slate-600 mt-1 flex items-center gap-1">
              <Clock size={9} /> Unlocks after:{" "}
              {slot.unlock.map((e) => CHECKIN_EVENT_LABELS[e]).join(", ")}
            </div>
          )}
        </div>
      </div>
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          {slotPhotos.slice(0, 6).map((p) => (
            <button
              key={p.id}
              onClick={() => openDocument(p.url)}
              className="w-10 h-10 rounded-lg overflow-hidden text-left"
              style={{ border: "1px solid rgba(255,255,255,0.1)" }}
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
...
function LiveTimer({
  ...
  return (
    <div className="flex items-center justify-between p-3.5 rounded-xl border border-amber-500/20 bg-amber-500/5 animate-pulse mb-3">
      ...
      <span className={`font-mono font-black text-sm ${colorClass}`}>
        {elapsed}
      </span>
    </div>
  );
...
function ActiveLoadCard({ load, onRefresh }: ActiveLoadCardProps) {
  ...
  const refreshDetail = useCallback(async () => {
    const [cis, ph] = await Promise.all([
      getCheckins(load.id),
  ...
  const hasDetentionRunning =
    doneEvents.has("detention_start") && !doneEvents.has("detention_end");
  const hasLayoverRunning =
    doneEvents.has("layover_start") && !doneEvents.has("layover_end");
    ...
    try {
      ...
      if (!result.ok) {
        if (
          ...
          result.error.includes("BOL document")
        ) {
          setActiveTab("docs");
          toast.error(
            "Upload the signed BOL before marking the load as departed.",
          );
          return;
        }
        if (event === "delivered" && result.error.includes("POD document")) {
          setActiveTab("docs");
          toast.error(
            "Upload the signed POD before marking the load as delivered.",
          );
          return;
        }
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div>
            <div className="text-[11px] text-violet-600 font-semibold tracking-widest uppercase mb-0.5">
              Active assignment
            </div>
        ...
        <div className="px-5 py-4">
                  ...
                  <div className="text-slate-500 text-[11px] mt-0.5 flex items-center gap-1">
                    ...
                    {load.pickupApptNumber && (
                      <span className="text-amber-500/70">
                        {" "}
                        · Appt #{load.pickupApptNumber}
                      </span>
                    )}
                  </div>
                </div>
                <a
                  href={`geo:0,0?q=${encodeURIComponent(`${load.pickupFacility} ${load.pickupAddress || ""} ${load.pickupCity} ${load.pickupState} ${load.pickupZip || ""}`)}(${encodeURIComponent(load.pickupFacility.replace(/[()]/g, ""))})`}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-slate-500 text-[11px] mt-0.5 flex items-center gap-1">
                    ...
                    {load.deliveryApptNumber && (
                      <span className="text-amber-500/70">
                        {" "}
                        · Appt #{load.deliveryApptNumber}
                      </span>
                    )}
                  </div>
                </div>
                <a
                  href={`geo:0,0?q=${encodeURIComponent(`${load.deliveryFacility} ${load.deliveryAddress || ""} ${load.deliveryCity} ${load.deliveryState} ${load.deliveryZip || ""}`)}(${encodeURIComponent(load.deliveryFacility.replace(/[()]/g, ""))})`}
                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[10px] font-bold text-violet-700 bg-violet-50 border border-violet-200 hover:bg-violet-100 transition-colors shrink-0"
                >
                  <Navigation size={10} /> Navigate
                </a>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <div className="rounded-md px-3 py-2.5 bg-slate-50 border border-slate-200">
              <div className="text-slate-500 text-[10px] uppercase tracking-widest flex items-center gap-1 mb-0.5">
                <Weight size={10} />
                Cargo
              </div>
            ...
            <div className="rounded-md px-3 py-2.5 bg-slate-50 border border-slate-200">
              ...
              <div className="text-slate-400 text-xs">
                {load.brokerContact || load.brokerPhone}
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 rounded-md px-4 py-3 bg-violet-50 border border-violet-100">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-violet-600 text-xs mb-0.5">Your Net</div>
                <div className="text-violet-700 text-xl font-black font-mono">
                  {fmt$(load.carrierNet)}
                </div>
              </div>
            </div>
          ...
          {load.notes && (
            <div
              className="mt-3 px-3 py-2.5 rounded-xl flex gap-2"
              style={{
        ...
        <div style={{ borderTop: "1px solid #e2e8f0" }}>
          <div className="flex" style={{ borderBottom: "1px solid #e2e8f0" }}>
            {[
              ...
              {
                id: "docs",
                label: `Documents & Photos${uploadedCount > 0 ? ` (${uploadedCount})` : ""}`,
              },
            ].map((tab) => (
              <button
                key={tab.id}
          ...
          {activeTab === "checkin" && (
            <div className="px-5 py-5 space-y-4">
              ...
              {doneEvents.has("arrived_pickup") &&
                !doneEvents.has("loaded_departing") &&
                checkins.find((c) => c.event === "arrived_pickup") && (
                  <LiveTimer
                    startTime={
                      checkins.find((c) => c.event === "arrived_pickup")!
                        .timestamp
              ...
              {doneEvents.has("arrived_delivery") &&
                !doneEvents.has("delivered") &&
                checkins.find((c) => c.event === "arrived_delivery") && (
                  <LiveTimer
                    startTime={
                      checkins.find((c) => c.event === "arrived_delivery")!
                        .timestamp
              ...
              {hasDetentionRunning &&
                checkins.find((c) => c.event === "detention_start") && (
                  <LiveTimer
                    startTime={
                      checkins.find((c) => c.event === "detention_start")!
                        .timestamp
              ...
              {hasLayoverRunning &&
                checkins.find((c) => c.event === "layover_start") && (
                  <LiveTimer
                    startTime={
                      checkins.find((c) => c.event === "layover_start")!
                        .timestamp
                ...
                <div className="flex gap-1.5">
                  {CORE_CHECKIN_ORDER.map((ev) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-full h-1.5 rounded-full transition-all ${done ? "bg-amber-500" : isCurrent ? "bg-amber-500/30" : "bg-slate-700"}`}
                        />
                        <div
                          className={`text-[9px] text-center ${done ? "text-amber-400" : isCurrent ? "text-amber-500/60" : "text-slate-600"}`}
                        >
                          {ev === "arrived_pickup"
                            ? "Pickup"
                            : ev === "loaded_departing"
              ...
              {nextEvent && (
                <button
                  onClick={() => {
                    if (nextEvent === "loaded_departing" && !load.bolUrl) {
                      setActiveTab("docs");
                      toast.error(
                        "Upload the signed BOL before departing pickup.",
                      );
                      return;
                    }
                    if (nextEvent === "delivered" && !load.podUrl) {
                      setActiveTab("docs");
                      toast.error(
                        "Upload the signed POD before marking delivery complete.",
                      );
                      return;
                    }
                    [
                ...
                >
                  {React.createElement(CHECKIN_CONFIGS[nextEvent].Icon, {
                    size: 22,
                  })}
                  {loading
                    ? "Logging..."
                    : nextEvent === "loaded_departing" && !load.bolUrl
                      ? "Upload BOL to Continue"
                      : nextEvent === "delivered" && !load.podUrl
                        ? "Upload POD to Continue"
                        : CHECKIN_CONFIGS[nextEvent].label}
                </button>
              )}
              {!nextEvent && doneEvents.has("delivered") && (
                <div
                  className="w-full rounded-2xl flex items-center justify-center gap-2 py-4 text-emerald-400 font-semibold"
                  style={{
                ...
                <div>
                  ...
                  >
                    <span className="font-semibold">
                      Log: Detention · Layover · TONU · Breakdown · Accident
                    </span>
                    <ChevronDown
                      size={14}
                      className={`transition-transform ${showOtherEvents ? "rotate-180" : ""}`}
                    />
                  </button>
                  {showOtherEvents && (
            ...
            <div className="px-5 py-5 space-y-5">
              ...
              {load.ratConUrl && (
                ...
                >
                  ...
                  <button
                    onClick={() => openDocument(load.ratConUrl ?? "")}
...
function PastLoadRow({ load }: { load: SonexLoad }) {
  return (
    <Link
      href={`/carrier/loads?load=${encodeURIComponent(load.id)}`}
      className="flex items-center gap-3 rounded-xl px-4 py-3.5 transition-colors hover:border-blue-300 hover:bg-blue-50"
    ...
    >
      <div className="flex-1 min-w-0">
        ...
        <div className="text-slate-500 text-xs mt-0.5">
          {fmtDate(load.pickupDate)} · {load.miles.toLocaleString()} mi
        </div>
      </div>
      <div className="text-right flex-shrink-0 mr-1">
        <div className="text-slate-400 text-xs">
          Gross:{" "}
          <span className="text-slate-300 font-mono">{fmt$(load.rate)}</span>
        </div>
        <div className="text-amber-400 text-base font-bold font-mono">
          {fmt$(load.carrierNet)}
        </div>
...
export function CarrierLoadsWorkspace() {
  const { user } = useSonexAuth();
  ...
  const selectedLoad = selectedLoadId
    ...
    : undefined;
  const displayedLoad = selectedLoad ?? activeLoad;
  ...
  return (
    <div
      ...
      className="mx-auto max-w-[1180px] px-3 py-5 sm:px-5 space-y-5"
    >
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            Active assignment and history
          </p>
        </div>
        <div className="flex items-center gap-2">
          <a
            href="tel:(346)421-2681"
      ...
      {!carrierId ? (
        <div className="flex flex-col items-center justify-center py-20 px-6 text-center rounded-2xl border border-white/[0.06] bg-white/[0.01]">
          ...
          <h3 className="text-white text-base font-bold mb-1">
            Carrier Link Required
          </h3>
          <p className="text-slate-400 text-xs max-w-sm leading-relaxed">
            This account is not linked to any carrier profile in the database.
            Please select a carrier from the simulator above to preview the
            portal features.
          </p>
        </div>
      ) : initialLoading ? (
        ...
        <ActiveLoadCard load={displayedLoad} onRefresh={refresh} />
      ) : (
...
function NewCarrierLoadModal({
  carrierId,
  carrierFee,
  onClose,
  onSaved,
}: NewCarrierLoadModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div
        className="relative w-full max-w-2xl h-full flex flex-col animate-slide-in-right"
        style={{
          background: "#080B14",
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              {input("Broker Name", "brokerName", "text", "C.H. Robinson")}
            </div>
          ...
          <div className="border-t border-white/5 pt-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                {input("Commodity", "commodity", "text", "Steel Coils")}
              </div>
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-white/[0.06] bg-[#03060E] shrink-0">
          ...
          <button
            onClick={handleSave}
            className="px-5 py-2 rounded-xl text-xs font-black bg-amber-500 text-black hover:bg-amber-400 transition-all"
          >
            Create Load
          </button>
        </div>
