    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div data-sonex-load-modal className="relative flex h-full w-full max-w-[720px] flex-col border-l border-slate-200 bg-[#f4f7fb] shadow-2xl">
        <div className="relative flex shrink-0 items-start justify-between overflow-hidden border-b border-slate-800 bg-[#101a2f] px-7 py-6">
          ...
          <div>
            <div className="flex items-center gap-3"><span className="inline-flex size-9 items-center justify-center rounded-lg bg-blue-500 text-white shadow-lg shadow-blue-950/30"><Plus size={18} /></span><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-blue-300">Load operations</p><h3 className="mt-0.5 text-xl font-bold text-white">Create New Load</h3></div></div>
            <p className="mt-3 text-xs text-slate-300">Choose a document-assisted workflow or enter dispatch details directly.</p>
          </div>
          <button onClick={onClose} className="rounded-md p-2 text-slate-400 transition-colors hover:bg-white/10 hover:text-white" title="Close new load">
            <X size={18} />
          </button>
        </div>

        <div className="grid shrink-0 grid-cols-2 gap-2 border-b border-slate-200 bg-white px-7 py-4">
          {([
            ['ai', 'AI parse document'],
            ['manual', 'Manual entry'],
          ] as const).map(([id, label]) => <button key={id} onClick={() => setCreationMode(id)} className={`flex h-12 items-center justify-center gap-2 rounded-md border text-xs font-semibold transition-colors ${creationMode === id ? 'border-blue-600 bg-blue-600 text-white shadow-sm shadow-blue-100' : 'border-slate-200 bg-slate-50 text-slate-600 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700'}`}>{id === 'ai' ? <Sparkles size={15} /> : <FileText size={15} />}{label}</button>)}
        ...
        <div className="flex-1 overflow-y-auto px-7 py-6">
          {creationMode === 'ai' ? (
            <div className="mx-auto max-w-[580px] space-y-5 py-2">
              <div className="border border-slate-800 bg-[#15223a] p-5 text-white shadow-sm"><div className="flex items-start gap-3"><span className="inline-flex size-9 shrink-0 items-center justify-center rounded-md bg-violet-500 text-white"><Sparkles size={18} /></span><div><p className="text-sm font-semibold">Document-assisted load creation</p><p className="mt-1 text-xs leading-5 text-slate-300">Sonex extracts the document into an editable review screen. Nothing is created until you check and save it.</p></div></div></div>
              <div><p className="mb-2 text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">1. Select paperwork</p><div className="grid grid-cols-2 gap-3"><button type="button" onClick={() => setAiDocumentType('rate_confirmation')} className={`relative border p-4 text-left transition-colors ${aiDocumentType === 'rate_confirmation' ? 'border-blue-600 bg-blue-50 ring-1 ring-blue-600' : 'border-slate-200 bg-white hover:border-blue-300'}`}><span className={`mb-3 inline-flex size-8 items-center justify-center rounded-md ${aiDocumentType === 'rate_confirmation' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}><FileText size={16} /></span><p className="text-xs font-bold text-slate-900">Rate confirmation</p><p className="mt-1 text-[11px] leading-4 text-slate-500">Tender, rate con, or broker order</p>{aiDocumentType === 'rate_confirmation' && <Check className="absolute right-3 top-3 text-blue-600" size={15} />}</button><button type="button" onClick={() => setAiDocumentType('bol')} className={`relative border p-4 text-left transition-colors ${aiDocumentType === 'bol' ? 'border-violet-600 bg-violet-50 ring-1 ring-violet-600' : 'border-slate-200 bg-white hover:border-violet-300'}`}><span className={`mb-3 inline-flex size-8 items-center justify-center rounded-md ${aiDocumentType === 'bol' ? 'bg-violet-600 text-white' : 'bg-slate-100 text-slate-500'}`}><Package size={16} /></span><p className="text-xs font-bold text-slate-900">Bill of lading</p><p className="mt-1 text-[11px] leading-4 text-slate-500">Shipment or BOL paperwork</p>{aiDocumentType === 'bol' && <Check className="absolute right-3 top-3 text-violet-600" size={15} />}</button></div></div>
              <div><p className="mb-2 text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">2. Upload document</p><button type="button" disabled={aiParsing} onClick={() => aiFileRef.current?.click()} className="group flex min-h-52 w-full flex-col items-center justify-center border-2 border-dashed border-slate-300 bg-white px-6 text-center transition-colors hover:border-blue-500 hover:bg-blue-50 disabled:cursor-wait disabled:opacity-60"><span className="mb-3 inline-flex size-12 items-center justify-center rounded-full border border-emerald-200 bg-emerald-100 text-emerald-700 ring-8 ring-emerald-100/70">{aiParsing ? <Loader2 className="animate-spin" size={21} /> : <UploadCloud size={21} />}</span><span className="text-sm font-semibold text-slate-900">{aiParsing ? 'Reading document...' : 'Select a document'}</span><span className="mt-1 text-xs text-slate-500">PDF, JPG, PNG, or WEBP up to 15 MB</span><span className="mt-4 bg-slate-900 px-4 py-2 text-xs font-semibold text-white transition-colors group-hover:bg-blue-600">Browse files</span></button></div>
              <div className="flex items-center gap-2 border border-emerald-100 bg-emerald-50 px-3 py-2.5 text-[11px] text-emerald-800"><Check size={14} className="shrink-0" />Review every extracted field before it becomes a load.</div>
            </div>
